...
同样,这个被依赖的gradle plugin也会被下载到本地,保存在$ADNROID_STUDIO_HOME/gradle/m2repository/com/android/tools/build/gradle/目录下.每一个Android studio目录下都会默认自带一个gradle文件夹,该文件夹包含一个默认版本的gradle,后续编译中用到的gradle plugin就保存在这里,所以该文件夹不可以随意移动.  
