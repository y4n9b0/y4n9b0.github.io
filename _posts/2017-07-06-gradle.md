...
# Prologue
借用老罗的话说,这可能也许大概是目前史上网上能搜到的最全最通俗易懂的[Gradle](https://gradle.org/)介绍了.  
...
android.applicationVariants.all { variant ->
    ...
    if (variant.install != null) {
        variant.install.doLast {
            ...
            def appId = variant.productFlavors.applicationId[-1] ?: android.defaultConfig.applicationId
...
欢迎个人或者商业使用,但请注明出处.@明子,不要再cd到output目录使用adb install了.

# Epilogue
关于gradle,就到此结束了,如果你还有什么感兴趣的,可以留言(需要登录Github账号)共同探讨.  
当然,鉴于能力所限,若有错误之处,还望不吝指正.
